...
def get_most_probable_tree_paths(tree, feature_names):
    paths = get_tree_paths(tree, feature_names)
    ...
    return paths.head(5)[['segment','leaf_support','class_1_proba']]

def get_most_probable_ensemble_paths(model, feature_names):
    paths = []
    for tree_i in model.estimators_:
        paths += get_tree_paths(tree_i, feature_names)
